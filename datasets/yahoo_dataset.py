...
from datasets.common import get_random_indices, get_target_indices, poison_text_dataset
...
def load_two_party_data(data_dir, args):
    ...
    temp = [i for i in range(len(sr_X_train) - 1000, len(sr_X_train))]
    ...
    sr_X_train, sr_ba_backdoor_indices_train = poison_text_dataset(sr_X_train, sr_y_train, sr_ba_backdoor_target_indices,args)
    ...
    return X_train, y_train, X_test, y_test, backdoor_y_train, backdoor_X_test, backdoor_y_test, \
           ...
           sr_X_train, sr_y_train, sr_ba_backdoor_target_indices
...
def generate_dataloader(data_list, batch_size, tokenizer, shuffle=True, backdoor_indices=None, trigger=None):
    """
    ...
    ds = LabeledTextDataset(dataset_text=X,
                            ...
                            tokenizer=tokenizer,
                            max_seq_len=256,
                            backdoor_indices=backdoor_indices, trigger=trigger)
...
def get_yahoo_dataloader(args):
    ...
    backdoor_indices, backdoor_target_indices, labeled_data_list, unlabeled_data_list, train_unlabeled_indices, \
    sr_X_train, sr_y_train, sr_ba_backdoor_target_indices = result
    ...
    s_r_train_dl = generate_dataloader((sr_X_train, sr_y_train), batch_size, tokenizer,
                                       shuffle=False,
