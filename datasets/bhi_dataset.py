...
def load_parties_data(data_dir, args):
    ...
    sr_ba_backdoor_target_indices = get_target_indices(sr_y_train, args['backdoor_label'], args['backdoor_train_size'],
                                                       backdoor_indices=temp)
    sr_ba_backdoor_target_indices.sort()
    sr_ba_source_indices_train = []
    length = len(sr_y_train)
    while len(sr_ba_source_indices_train) < len(sr_ba_backdoor_target_indices):
        i = sr_ba_backdoor_target_indices[len(sr_ba_source_indices_train)]
        j = random.randint(i+1, length-1)
        if j not in sr_ba_source_indices_train and sr_y_train[j] != args['backdoor_label']:
            sr_ba_source_indices_train.append(j)
    sr_ba_source_indices_train.sort()
    ...
    return X_train, y_train, X_test, y_test, backdoor_y_train, backdoor_X_test, backdoor_y_test, \
        ...
        sr_X_train, sr_y_train, sr_ba_backdoor_target_indices,sr_ba_source_indices_train

def generate_dataloader(data_list, batch_size, transform, shuffle=True, backdoor_indices=None, party_num=2, trigger=None, trigger_add=None, source_indices=None):
    """
...
def get_bhi_dataloader(args):
    ...
    sr_X_train, sr_y_train, sr_ba_backdoor_target_indices,sr_ba_source_indices_train = result
    ...
    s_r_train_dl = generate_dataloader((sr_X_train, sr_y_train), batch_size, transform,
                                       shuffle=False, backdoor_indices=sr_ba_backdoor_target_indices, party_num=party_num,
                                       trigger=args['trigger'], trigger_add=args['trigger_add'], source_indices=sr_ba_source_indices_train)
