...
def mix_match_train(labeled_loader, unlabeled_loader, bottom_model, top_model,
    ...
    for batch_idx in range(args['lr_ba_top_train_iteration']):
        try:
            inputs, targets_x, _, _ = labeled_iter.next()
        ...
        try:
            inputs, _, _, _ = unlabeled_iter.next()
        except StopIteration:
            unlabeled_iter = iter(unlabeled_loader)
            inputs, _, _, _ = unlabeled_iter.next()
        if args['dataset'] != 'bhi' and args['n_passive_party'] < 2:
...
def train_top_model_by_mix_match(train_loader, test_loader, labeled_loader, unlabeled_loader, bottom_model, args):
    ...
    if not args['time']:
        train_acc = predict(loader=train_loader,
                            ...
                            num_classes=args['num_classes'],
                            dataset=args['dataset'],
                            party_num=args['n_passive_party'])
...
def predict(loader, bottom_model, top_model, use_cuda, num_classes, dataset, top_k=1, party_num=1):
    ...
    y_true = []
    with torch.no_grad():
        for batch_idx, (batch_inputs, targets, _, _) in enumerate(loader):
            if dataset != 'bhi' and party_num < 2:
