...
def add_from_unlabeled(bottom_model, top_model, args, unlabeled_loader, train_size, top_k=1):
    ...
    if args['dataset'] != 'yahoo':
        for inputs, _, _, _ in unlabeled_loader:
            if args['dataset'] != 'bhi' and args['n_passive_party'] < 2:
                _, X = inputs
            else:
                X = inputs[:, 1]
...
def get_original_backdoor_output(labeled_loader, bottom_model, top_model, args, unlabeled_loader, top_k=1,
    ...
    for _, (inputs, label, _,  _) in enumerate(labeled_loader.dataset):
        if args['dataset'] != 'bhi' and args['n_passive_party'] < 2:
            _, X = inputs
            if args['dataset'] == 'yahoo':
                label = label[0]
...
def generate_backdoor_output(labeled_loader, bottom_model, top_model, args, unlabeled_loader=None, top_k=1,
    ...
    # result 1,x,x,x
    logging.info('result shape: {}'.format(result.shape))
    logging.info("--- start backdoor output")
    ...
    different_norm = torch.norm(old_result-result, p=2)

    logging.info('differnet norm: {}'.format(different_norm))

    # return result.data
    return result_data_list, different_norm
...
def finetune_bottom_model(old_bottom_model, backdoor_output_list, train_loader, backdoor_indices, args):
    ...
    for backdoor_output in backdoor_output_list:
        ...
        if len(X) == 0:
            for _, (inputs, _, _, indices) in enumerate(train_loader):
                if args['dataset'] != 'bhi' and args['n_passive_party'] < 2:
                    _, Xb_inputs = inputs
                else:
                    Xb_inputs = inputs[:, 1]
        ...
        if len(backdoor_output.shape) == 4:
            backdoor_y = backdoor_output.cpu().repeat(backdoor_X.shape[0], 1, 1, 1).detach()
        elif len(backdoor_output.shape) == 2:
        ...
        for ep in range(args['lr_ba_finetune_epochs']):
            ...
            loss = sum(loss_list)
            logging.info('epoch: {}, loss: {}'.format(ep, loss/len(loss_list)))
            if use_wandb:
                wandb.log({'finetune loss':loss/len(loss_list)})
...
def poison_predict(
    ...
    with torch.no_grad():
        for target in target_list:
            for batch_idx, (inputs, targets, _, indices) in enumerate(test_loader):
                party_X_test_dict = dict()
                if dataset != 'bhi' and args['n_passive_party'] < 2:
                ...
                if len(target.shape) == 4:
                    y_prob_preds = vfl.predict(Xa_inputs, party_X_test_dict, target.repeat(Xa_inputs.shape[0], 1, 1, 1))
...
def lr_ba_backdoor(train_loader, test_loader, backdoor_indices,
    ...
    if not load:
        ...
        if lr_ba_top_model is None:
            lr_ba_top_model = train_top_model(train_loader=train_loader,
                                            ...
                                            bottom_model=bottom_model,
                                            args=args)
        # print(lr_ba_top_model)
        # print execution time of model completion
        if args['time']:
            start_time = print_running_time('model completion', start_time)

        # generate backdoor latent representation
        backdoor_output_list, different_norm = generate_backdoor_output(labeled_loader=labeled_loader,
                                                        bottom_model=bottom_model,
    ...
    return new_bottom_model_list, lr_ba_top_model, backdoor_output_list
