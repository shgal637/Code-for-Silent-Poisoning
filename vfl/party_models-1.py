...
class VFLActiveModel(object):
    ...
    def _fit(self, X, y):
        ...
        # self.K_U = self.bottom_model.forward(X)
    ...
    def _compute_common_gradient_and_loss(self, y):
        ...
        if not self.top_trainable:
            ...
            temp = torch.cat(grad_comp_list, 1)  # [, 20]
            U = self.top_model.forward(temp)

        # compute loss
        y = y.long().to(U.device)  # [, 10]
        class_loss = self.classifier_criterion(U, y)

        # compute gradients
        if self.top_trainable:
            self.top_model.my_backward(loss=class_loss)
            grad_list = [temp.grad for temp in grad_comp_list]  # [2, 32, 10]
        else:
    ...
    def _update_models(self, X, y):
        ...
        loss = torch.sum(self.top_grads * self.K_U)
        self.bottom_model.my_backward(loss=loss)
...
class VFLPassiveModel(object):
    ...
    def backward(self):
        loss = torch.sum(self.common_grad * self.components)
        self.bottom_model.my_backward(loss=loss)
